...
def DGeqsys(fes,U0,v0,sig0,c,gD,fullsys=False, applyrhs = False):
    ...
    if(applyrhs == False):
        ...
        rhs += SymbolicBFI( ( pow(c,-2)*v*w + sig*tau ), BND,definedon=fes.mesh.Boundaries("inflow"), skeleton=True) #t=0 (or *(1-x))
        rhs += SymbolicBFI( ( gD * (alpha*w - tau*n_x) ), BND, definedon=fes.mesh.Boundaries("dirichlet"), skeleton=True) #dirichlet boundary 'timelike'
        rhs += SymbolicBFI( gamma * ( (U)*V ) ,BND, definedon=fes.mesh.Boundaries("inflow"),  skeleton=True ) #rhs correction term to recover sol of second order system
