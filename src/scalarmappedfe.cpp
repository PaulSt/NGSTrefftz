...
namespace ngfem
{
    ...
    Complex BaseScalarMappedElement ::
...
//#endif
    ...
    template<>
    void ScalarMappedElement<1> :: CalcMappedDDShape (const BaseMappedIntegrationPoint & bmip,
                                                BareSliceMatrix<> hddshape) const
    {;}

    template<>
    void ScalarMappedElement<2> :: CalcMappedDDShape (const BaseMappedIntegrationPoint & bmip,
                                                BareSliceMatrix<> hddshape) const
    {
            ...
            for(int d2=0;d2<2;d2++)
            {
                ...
                for (int i=0; i<this->ndof; ++i)
                {
                    ...
                    for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                        ddshape(i,d2*2+d1) += (localmat)[2][j]*pol[(localmat)[1][j]] * pow(1.0/elsize,2);
                }
            }
        }
    }
    ...
    template<>
    void ScalarMappedElement<3> :: CalcMappedDDShape (const BaseMappedIntegrationPoint & bmip,
                                                BareSliceMatrix<> hddshape) const
    {
        ...
        for(int d1=0;d1<3;d1++)
        {
            for(int d2=0;d2<3;d2++)
            {
                ...
                for (int i=0; i<this->ndof; ++i)
                {
                    ...
                    for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                        ddshape(i,d2*3+d1) += (localmat)[2][j]*pol[(localmat)[1][j]] * pow(1.0/elsize,2);
                }
            }
        }
    }
                                        ...
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            cpoint -= elcenter; cpoint *= (1.0/elsize); cpoint[1] *= c;
                                        ...
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            cpoint -= elcenter; cpoint *= (1.0/elsize); cpoint[2] *= c;
            // calc 1 dimensional monomial basis
                                        ...
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        ...
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            cpoint -= elcenter; cpoint *= (1.0/elsize); cpoint[3] *= c;
            // calc 1 dimensional monomial basis
        ...
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            for(int d=0;d<2;d++)
            {
                ...
                for (int i=0; i<this->ndof; ++i)
                {
                    ...
                    for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                        dshape(i*2+d,imip) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==1 ? c : 1) * (1.0/elsize);
                }
            }
        }
        ...
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            for(int d=0;d<3;d++)
            {
                ...
                for (int i=0; i<this->ndof; ++i)
                {
                    ...
                    for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                        dshape(i*3+d,imip) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==2 ? c : 1) * (1.0/elsize);
                }
            }
        }
        //dshape *= (2.0/elsize); //inner derivative
        ...
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            for(int d=0;d<4;d++)
            {
                ...
                for (int i=0; i<this->ndof; ++i)
                {
                    ...
                    for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                        dshape(i*4+d,imip) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==3 ? c : 1) * (1.0/elsize);
                }
            }
        }
                                         ...
                                         BareSliceMatrix<> dshape) const
    {
        ...
        for(int d=0;d<2;d++)
        {
            ...
            for (int i=0; i<this->ndof; ++i)
            {
                dshape(i,d) = 0.0;
                for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                    dshape(i,d) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==1 ? c : 1) * (1.0/elsize);
            }
        }
        //dshape *= (2.0/elsize); //inner derivative
                                         ...
                                         BareSliceMatrix<> dshape) const
    {
        ...
        for(int d=0;d<3;d++)
        {
            ...
            for (int i=0; i<this->ndof; ++i)
            {
                dshape(i,d) = 0.0;
                for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                    dshape(i,d) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==2 ? c : 1) * (1.0/elsize);
            }
        }
        //dshape *= (2.0/elsize); //inner derivative
                                         ...
                                         BareSliceMatrix<> dshape) const
    {
        ...
        for(int d=0;d<4;d++)
        {
            ...
            for (int i=0; i<this->ndof; ++i)
            {
                dshape(i,d) = 0.0;
                for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                    dshape(i,d) += (localmat)[2][j]*pol[(localmat)[1][j]] * (d==3 ? c : 1) * (1.0/elsize);
            }
        }
        //dshape *= (2.0/elsize); //inner derivative
    ...
    template<>
    void ScalarMappedElement<1> :: CalcDDWaveOperator (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape,
                                         BareSliceMatrix<SIMD<double>> wavespeed,
                                         BareSliceMatrix<SIMD<double>> mu) const
    {;}
    template<>
    void ScalarMappedElement<4> :: CalcDDWaveOperator (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape,
                                         BareSliceMatrix<SIMD<double>> wavespeed,
                                         BareSliceMatrix<SIMD<double>> mu) const
    {;}
    ...
    template<>
    void ScalarMappedElement<2> :: CalcDDWaveOperator (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape,
                                         BareSliceMatrix<SIMD<double>> wavespeed,
                                         BareSliceMatrix<SIMD<double>> mu) const
    {
        ...
        {
            Vec<2,SIMD<double>> cpoint = smir[imip].GetPoint();
            cpoint -= elcenter; cpoint *= (1.0/elsize);

            STACK_ARRAY(SIMD<double>, mem, 2*(order+1)+2); mem[0]=0;mem[1]=0;
            Vec<2,SIMD<double>*> polxt;
            for(int d=0;d<2;d++)
            {
            ...
            for (int i=0; i<this->ndof; ++i)
            {
                ...
                for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                    dshape(i*2+1,imip) += (localmat)[2][j]*pol[(localmat)[1][j]] * pow(1.0/elsize,2);
            }
        }
    }
    ...
    template<>
    void ScalarMappedElement<3> :: CalcDDWaveOperator (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape,
                                         BareSliceMatrix<SIMD<double>> wavespeed,
                                         BareSliceMatrix<SIMD<double>> mu) const
    {
        for (size_t imip = 0; imip < smir.Size(); imip++)
        {
            ...
            STACK_ARRAY(SIMD<double>, mem, 3*(order+1)+2); mem[0]=0;mem[1]=0;
            Vec<3,SIMD<double>*> polxt;
            for(int d=0;d<3;d++)
            {
                polxt[d] = &mem[d*(order+1)+2];
            ...
            for (int i=0; i<this->ndof; ++i)
            {
                ...
                for (int j=(localmat)[0][i]; j<(localmat)[0][i+1]; ++j)
                    dshape(i*3+2,imip) += (localmat)[2][j]*pol[(localmat)[1][j]] * pow(1.0/elsize,2);
            }
        }
    }
    ...
    //template<int D>
    //BlockMappedElement<D> :: BlockMappedElement(int aord, float ac, Vec<D> aelcenter, double aelsize, ELEMENT_TYPE aeltype)
    //: ord(aord),
    //c(ac),
    ...
    template<int D>
    void BlockMappedElement<D> :: CalcShape (const SIMD_BaseMappedIntegrationRule & smir,
                                        BareSliceMatrix<SIMD<double>> shape) const
    ...
    template<>
    void BlockMappedElement<1> :: CalcDShape (const BaseMappedIntegrationPoint & mip,
                                         BareSliceMatrix<> dshape) const
    { throw Exception("Not implemented point "); }
    template<>
    void BlockMappedElement<4> :: CalcDShape (const BaseMappedIntegrationPoint & mip,
                                         BareSliceMatrix<> dshape) const
    { throw Exception("Not implemented point "); }

    template<>
    void BlockMappedElement<2> :: CalcDShape (const BaseMappedIntegrationPoint & mip,
                                         BareSliceMatrix<> dshape) const
    {
        Vec<2> cpoint = mip.GetPoint();
        cpoint -= elcenter; cpoint *= (1.0/elsize); cpoint[1] *= c;
    ...
    template<>
    void BlockMappedElement<3> :: CalcDShape (const BaseMappedIntegrationPoint & mip,
                                         BareSliceMatrix<> dshape) const
    {
        Vec<3> cpoint = mip.GetPoint();
        cpoint -= elcenter; cpoint *= (1.0/elsize); cpoint[2] *= c;
    ...
    //template<int D>
    //void BlockMappedElement<D> :: CalcDShape (const BaseMappedIntegrationRule & mir,
                                         //BareSliceMatrix<> dshapes) const
    //{ throw Exception("Not implemented no simd "); }

    template<>
    void BlockMappedElement<1> :: CalcDShape (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape) const
    { throw Exception("Not implemented 1d"); }

    template<>
    void BlockMappedElement<2> :: CalcDShape (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape) const
    {
    ...
    template<>
    void BlockMappedElement<3> :: CalcDShape (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape) const
    {
        for (int imip = 0; imip < smir.Size(); imip++)
        {
    ...
    template<>
    void BlockMappedElement<4> :: CalcDShape (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape) const
    {
        for (int imip = 0; imip < smir.Size(); imip++)
        {
    ...
    template<>
    Vec<2> PlainWaveElement<2> :: GetDirection(int i) const
    {
        //Array<Vec<2>> pwdir(ndof);
        //for(int i=0;i<ndof;i++)
        //{
        Vec<2> pwdir = { cos(2.0*M_PI*i/this->ndof), sin(2.0*M_PI*i/this->ndof) };
        //cout << pwdir << endl;
    ...
    template<>
    void PlainWaveElement<2> :: CalcDShape (const BaseMappedIntegrationPoint & mip, BareSliceMatrix<Complex> dshape) const
    {
        Vec<2> cpoint = mip.GetPoint();
        cpoint -= elcenter;
        ...
        for(int d=0;d<2;d++)
            for (int i=0; i<this->ndof; ++i)
            {
                Vec<2> dir = GetDirection(i);
                dshape(i,d) = 0.0;
                dshape(i,d) = 1i*dir[d]*exp(1i*(cpoint[0]*dir[0] + cpoint[1]*dir[1]));
