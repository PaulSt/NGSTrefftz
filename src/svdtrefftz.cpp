...
namespace ngcomp
{
  ...
  public:
    ...
    void AppendElmat (const Array<int> &dofs1, const Array<int> &dofs2,
                      FlatMatrix<> elmat)
    {
                                     ...
                                     shared_ptr<FESpace> fes, double eps)
  {
    ...
    ma->IterateElements (VOL, lh,
                         ...
                         // for(auto ei : ma->Elements())
                         {
                           ...
                           for (auto &bfi : bfis[VOL])
                             {
                               ...
                               elmat += elmati;
                               // bfi -> CalcElementMatrixAdd(fel, trafo,
                               // elmat, lh);
                             }
                           // cout << ei << " elmat = " << elmat << endl;
                           SliceMatrix<> A (elmat);
                           Matrix<double, ColMajor> U (dofs.Size (),
                                                       dofs.Size ()),
                           ...
                           // max_elsperrow =
                           // max_elsperrow<newdof.Size()?newdof.Size():max_elsperrow;
                           // cout << newdof.Size();
                           // cout << nnz <<  "SVD" << elmat.Diag() << endl <<
                           // U << endl << V << endl; FlatMatrix<> P =
                           // U.Cols(nnz,dofs.Size()); cout <<"P"<<endl<< P <<
                           // endl;
    ...
    // ElementByElementMatrix<double> P(fes->GetNDof(), 29, ma->GetNE(),
    // false); cout << fes->GetNDof() << ","<<max_elsperrow << endl;
    ...
    shared_ptr<SparseMatrixTM<double>> P
        = SparseMatrixTM<double>::CreateFromCOO (elmats.rowind, elmats.colind,
                                                 elmats.data, fes->GetNDof (),
                                                 Pwidth);
    return P;
  }
}
