...
namespace ngcomp
{
  ...
  public:
    ...
    /// sets up the space for the constrained Trefftz method.
    ///
    ///  @param op the differential operation. Bilinear from on `fes` x
    ///  `fes_test`.
    ///
    ///  @param fes_test the test finite element space of `op`
    ///
    ///  @param cop_lhs left hand side of the constraint operation. Bilinear
    ///  form on `fes` x `fes_constraint`.
    ///
    ///  @param cop_rhs right hand side of the constraint operation. Bilinear
    ///  form on `fes_constraint` x `fes_constraint`.
    ///
    ...
    /// `null`.
    shared_ptr<BaseVector> SetOp (shared_ptr<const SumOfIntegrals> op,
