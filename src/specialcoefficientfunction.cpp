    ...
    IntegrationPointFunction :: IntegrationPointFunction(shared_ptr<MeshAccess> mesh, IntegrationRule& intrule, Matrix<> ipdata)
                ...
                : CoefficientFunction(1)
            {
                ...
                for (size_t elnr=0;elnr<mesh->GetNE();elnr++)
                {
                    ...
                    Vector<> center(D); center = 0;
                    Vector<> v1(D);
                    auto vertices_index = mesh->GetElVertices(elnr);
                    switch(mesh->GetDimension())
                    {
                        case 2: for(auto vertex : vertices_index) center += mesh->GetPoint<2>(vertex); break;
                        case 3: for(auto vertex : vertices_index) center += mesh->GetPoint<3>(vertex); break;
                    }
        ...
        shared_ptr<IntegrationPointFunction>, CoefficientFunction>
            ...
            .def("__init__",
                 [](IntegrationPointFunction *instance,
                    shared_ptr<MeshAccess> mesh, IntegrationRule& intrule, Vector<> data)
                 {
