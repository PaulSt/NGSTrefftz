...
namespace ngcomp
{
    ...
    template<int D>
    void TLapBasis<D> :: TB_inner(int ord, Matrix<> &trefftzbasis, Vec<D, int> coeffnum, int basis, int dim, int &tracker, int basistype, double wavespeed)
    {
        if (dim>0)
        {
            while(coeffnum(dim-1)<=ord)
            ...
            if(sum<=ord)
            {
                ...
                else if(coeffnum(D-1)>1)
                {
                    for(int m=0;m<D-1;m++) //rekursive sum
                    {
                        ...
                        get_coeff[m] = get_coeff[m] + 2;
                        trefftzbasis( basis, indexmap) -= (coeffnum(m)+1) * (coeffnum(m)+2) * trefftzbasis(basis, PolBasis::IndexMap2<D-1>(get_coeff, ord));
                    }
                    trefftzbasis(basis, indexmap) *= wavespeed*wavespeed/(k * (k-1));
                }
            }
