...
namespace ngcomp
{
            ...
            case 1:
                {
                    if(eqtyp=="laplace")
                        ...
                        for(int d=0;d<D+1;d++) basismats[d]=FOTWaveBasis<1>::Basis(order, d);
            ...
            case 2:
                {
                    if(eqtyp=="laplace")
                        ...
                        basismats.SetSize(D+1);
                        for(int d=0;d<D+1;d++) basismats[d]=FOTWaveBasis<2>::Basis(order, d);
    ...
    void TrefftzFESpace :: SetWavespeed(shared_ptr<CoefficientFunction> awavespeedcf, shared_ptr<CoefficientFunction> aBBcf, shared_ptr<CoefficientFunction> aGGcf)
    {
        ...
        if(aBBcf || eqtyp=="qtwave" || eqtyp=="foqtwave")
        {
            ...
            timerder.Start();
            GGder.SetSize(this->order-(eqtyp=="qtwave"),pow(this->order-(eqtyp=="qtwavw"),D==2));

            for(int ny=0;ny<=(this->order-1-(eqtyp=="qtwave"))*(D==2);ny++)
            {
                ...
                case ET_TRIG:
                    {
                        ...
                        if(eqtyp=="helmholtz" || eqtyp=="helmholtzconj")
                        {
                            ...
                            return *(new (alloc) PlaneWaveElement<2>(local_ndof,order,eltype,ElCenter<1>(ei),1.0,c,(eqtyp=="helmholtz"?1:-1)));
                        }
                        else
            ...
            if(sum<=ord)
            {
                ...
                else if(coeffnum(D)>1)
                {
                    for(int m=0;m<D;m++) //rekursive sum
                    {
                        ...
                        trefftzbasis( basis, indexmap) += (coeffnum(m)+1) * (coeffnum(m)+2) * trefftzbasis(basis, PolBasis::IndexMap2<D>(get_coeff, ord));
                    }
    ...
    template<int D>
    void TLapBasis<D> :: TB_inner(int ord, Matrix<> &trefftzbasis, Vec<D+1, int> coeffnum, int basis, int dim, int &tracker, int basistype, double wavespeed)
    {
        if (dim>0)
        {
            while(coeffnum(dim-1)<=ord)
            ...
            if(sum<=ord)
            {
                ...
                else if(coeffnum(D)>0)
                {
                    ...
                    for(int d=0;d<D;d++)
                    {
                        ...
                        get_coeff[d] = get_coeff[d] + 1;
                        trefftzbasis[d](basis, indexmap) = (-1.0/k)*(coeffnum(d)+1) * trefftzbasis[D](basis, PolBasis::IndexMap2<D>(get_coeff, ord));
                        //rekursive sum
                        trefftzbasis[D]( basis, indexmap) += (-1.0/k)*(coeffnum(d)+1) * trefftzbasis[d](basis, PolBasis::IndexMap2<D>(get_coeff, ord));
                    }
                }
