...
namespace ngfem
{
    ...
    class ScalarMappedElement : public BaseScalarMappedElement
    {
        ...
        public:
            ...
            INLINE const FlatMatrixFixWidth<D> GetDShape (const BaseMappedIntegrationPoint & mip, LocalHeap & lh) const
            {
            ...
            virtual void CalcShape (const BaseMappedIntegrationPoint & mip, BareSliceVector<Complex> shape) const
            ...
            virtual void CalcDShape (const BaseMappedIntegrationPoint & mip, BareSliceMatrix<Complex> dshape) const
            ...
            virtual void CalcMappedDDShape (const BaseMappedIntegrationPoint & bmip, BareSliceMatrix<> hddshape) const;
            ...
            void Evaluate (const SIMD_BaseMappedIntegrationRule & mir, BareSliceVector<> coefs, BareVector<SIMD<double>> values) const
            {
                ...
                FlatMatrix<double> bdbmat(this->ndof,mir.Size()*nsimd,reinterpret_cast<double*>(&shape(0,0)));
                FlatVector<double> bdbvec(mir.Size()*nsimd,reinterpret_cast<double*>(&values(0)));
            ...
            void EvaluateGrad (const SIMD_BaseMappedIntegrationRule & ir, BareSliceVector<> coefs, BareSliceMatrix<SIMD<double>> values) const
            {
                ...
                const int nsimd = SIMD<double>::Size();
                FlatMatrix<double> dshapes(this->ndof,D*nsimd*ir.Size(),reinterpret_cast<double*>(&simddshapes(0,0)));
                FlatVector<double> bdbvec(D*nsimd*ir.Size(),reinterpret_cast<double*>(&values(0,0)));
                    ...
                    BareSliceVector<> coefs) const
            {
                ...
                const int nsimd = SIMD<double>::Size();
                FlatMatrix<double> dshapes(this->ndof,D*nsimd*mir.Size(),reinterpret_cast<double*>(&simddshapes(0,0)));
                FlatVector<double> bdbvec(D*nsimd*mir.Size(),reinterpret_cast<double*>(&values(0,0)));
    ...
    template <int D>
    class PlainWaveElement : public ScalarMappedElement<D>
    {
        private:
        ...
        public:
            PlainWaveElement(int andof, int aord, ELEMENT_TYPE aeltype, Vec<D> aelcenter = 0, double aelsize = 1, float ac = 1.0, int aconj=1) :
