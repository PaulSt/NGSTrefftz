...
namespace ngfem
{
  ...
  template <int D> void MyTrefftz<D>::TrefftzBasis ()
  {
    cout << "\n nbasis: " << nbasis << "\n";
  ...
  template <int D>
  void
  MyTrefftz<D>::MakeIndices (int maxes, vector<array<int, D + 1>> &indices)
  {
  ...
  template <int D>
  void MyTrefftz<D>::MakeIndices_inner (int dim, array<int, D + 1> &numbers,
                                        int maxes,
