...
public:
  ...
  // gradients of tent bottom at integration points (in possibly curved
  // elements)
...
{
public:
  GradPhiCoefficientFunction (int adim) : CoefficientFunction (adim) {}

  double Evaluate (const BaseMappedIntegrationPoint &ip) const
  {
    throw Exception (
        "Evaluate not implemented for BaseMappedIntegrationPoint!");
  }

  void Evaluate (const SIMD_BaseMappedIntegrationRule &mir,
                 BareSliceMatrix<SIMD<double>> values) const
  {
    // loads values of grad(phi) from ProxyUserData, assuming it is properly
    // set
    ProxyUserData &ud
...
public:
  ...
  template <int DIM>
  bool PitchTents (const double dt, const bool calc_local_ct,
                   const double global_ct = 1.0);

  // Get object features
  int GetNTents () { return tents.Size (); }
  int GetNLayers () { return nlayers + 1; }

  void SetMaxWavespeed (const double c)
  {
    cmax = make_shared<ConstantCoefficientFunction> (c);
  }
  ...
  void DrawPitchedTentsGL (Array<int> &tentdata, Array<double> &tenttimes,
                           int &nlevels);

  void SetPitchingMethod (ngstents::PitchingMethod amethod)
  {
    this->method = amethod;
  }
};
...
protected:
  ...
  shared_ptr<MeshAccess> ma;
  // element-wise (vol grad algo) or edge-wise (edge grad algo)  maximal
  // wave-speeds
  Array<double> cmax;
  ...
  // returns the mesh dependent local constants. the first parameter is the
  // vertex, and the second parameter is the edge (edge algo) or element (vol
  // algo)
  ...
  // Calculates the local c_tau used for ensuring causality (edge
  // algo)/preventing locks (vol algo)
  virtual Table<double>
  CalcLocalCTau (LocalHeap &lh, const Table<int> &v2e) = 0;
...
public:
  ...
  // This method precomputes mesh-dependent data. It includes the wavespeed
  // (per element) and neighbouring data. It returns the table v2v
  // (neighbouring vertices), v2e(edges adjacent to a given vertex) and
  // per_verts (used for periodicity).
  template <int DIM>
  std::tuple<Table<int>, Table<int>>
  InitializeMeshData (LocalHeap &lh, shared_ptr<CoefficientFunction> wavespeed,
  ...
  void
  ComputeVerticesReferenceHeight (const Table<int> &v2v, const Table<int> &v2e,
  ...
  Array<double> GetVerticesReferenceHeight ()
  {
    return Array<double> (vertex_refdt);
  }

  // Populate the set of ready vertices with vertices satisfying ktilde >
  // adv_factor * refdt. returns false if no such vertex was found
  [[nodiscard]] bool
  GetReadyVertices (double &adv_factor, bool reset_adv_factor,
                    const FlatArray<double> &ktilde,
                    const BitArray &complete_vertices, BitArray &vertex_ready,
  ...
  // guarantee causality
  virtual double GetPoleHeight (const int vi, const FlatArray<double> &tau,
                                FlatArray<int> nbv, FlatArray<int> nbe,
                                LocalHeap &lh) const = 0;

  // Returns the position in ready_vertices containing the vertex in which a
  // tent will be pitched (and its level)
  [[nodiscard]] std::tuple<int, int>
  PickNextVertexForPitching (const FlatArray<int> &ready_vertices,
                             const FlatArray<double> &ktilde,
...
template <int DIM> class VolumeGradientPitcher : public TentSlabPitcher
{
public:
...
template <int DIM> class EdgeGradientPitcher : public TentSlabPitcher
{
public:
