...
void GradPhiCoefficientFunction::GenerateCode (Code &code,
                                               FlatArray<int> inputs,
                                               int index) const
{
  ...
  variables["this"] = "reinterpret_cast<CoefficientFunction*>("
                      + code.AddPointer (this) + ")";

  variables["flatmatrix"]
      = code.is_simd ? "FlatMatrix<SIMD<double>>" : "FlatMatrix<double>";
  variables["values"] = Var ("values", index).S ();

  string scal_type = "double";
  if (code.is_simd)
    scal_type = "SIMD<" + scal_type + ">";
...
template <int DIM>
bool TentPitchedSlab::PitchTents (const double dt, const bool calc_local_ct,
                                  const double global_ct)
{
  if (has_been_pitched)
    {
      tents.DeleteAll ();
  ...
  TentSlabPitcher *slabpitcher = [this] () -> TentSlabPitcher * {
    switch (this->method)
      ...
      default:
        cout << "Trying to pitch tent without setting a pitching method."
             << endl;
        return nullptr;
        break;
      }
  ...
  // calc wavespeed for each element and perhaps other stuff (i..e, calculating
  // edge gradients, checking fine edges, etc)
  Table<int> v2v, v2e;
  std::tie (v2v, v2e) = slabpitcher->InitializeMeshData<DIM> (
      lh, cmax, calc_local_ct, global_ct);
  ...
  // whether to reset the adv_factor to its initial value after populating
  // ready_vertices
  ...
  while (!slab_complete)
    {
      ...
      const bool found_vertices = slabpitcher->GetReadyVertices (
          adv_factor, reset_adv_factor, ktilde, complete_vertices,
          vertex_ready, ready_vertices);
      ...
      while (ready_vertices.Size ())
        {
          ...
          std::tie (minlevel, posmin)
              = slabpitcher->PickNextVertexForPitching (ready_vertices, ktilde,
                                                        vertices_level);
          ...
          slabpitcher->UpdateNeighbours (vi, adv_factor, v2v, v2e, tau,
                                         complete_vertices, ktilde,
                                         vertex_ready, ready_vertices, lh);
          tents.Append (tent);
        }
  ...
  if (!slab_complete)
    {
      ...
      for (iv = 0; iv < ma->GetNV (); iv++)
        if (vmap[iv] == iv && !complete_vertices[iv])
          break;
      if (iv == ma->GetNV ()) // just as a precaution, let us check that it
                              // really didnt pitch.
        {
  ...
  ParallelFor (Range (tents), [&] (int i) {
    ...
    constexpr int n_vertices
        = DIM + 1; // number of vertices of the current element (simplex)
    ScalarFE<el_type, 1> fe; // finite element created for calculating the
...
template bool
TentPitchedSlab::PitchTents<1> (const double, const bool, const double);
template bool
TentPitchedSlab::PitchTents<2> (const double, const bool, const double);
template bool
TentPitchedSlab::PitchTents<3> (const double, const bool, const double);

double TentPitchedSlab::MaxSlope () const
{
  double maxgrad = 0.0;
  ParallelFor (Range (tents),
               [&] (int i) { AtomicMax (maxgrad, tents[i]->MaxSlope ()); });
  return maxgrad;
}

///////////////////// Pitching Algo Routines ///////////////////////////////
TentSlabPitcher::TentSlabPitcher (shared_ptr<MeshAccess> ama,
                                  ngstents::PitchingMethod m,
                                  Array<int> &avmap)
    : ma (ama), vertex_refdt (ama->GetNV ()), edge_len (ama->GetNEdges ()),
      local_ctau ([] (const int, const int) { return 1.; }), method (m),
      vmap (avmap)
{
  if (method == ngstents::PitchingMethod::EEdgeGrad)
    {
      cmax.SetSize (ma->GetNEdges ());
    }
  else
    {
      cmax.SetSize (ma->GetNE ());
    }
  cmax = -1;
}

bool TentSlabPitcher::GetReadyVertices (double &adv_factor,
                                        bool reset_adv_factor,
                                        ...
                                        Array<int> &ready_vertices)
{
  ...
  else if (adv_factor < 0.05)
    return false;
  return found;
}

void TentSlabPitcher::ComputeVerticesReferenceHeight (
    const Table<int> &v2v, const Table<int> &v2e, const FlatArray<double> &tau,
    LocalHeap &lh)
{
  ...
  for (auto i = 0; i < this->ma->GetNV (); i++)
    if (vmap[i] == i) // non-periodic
      {
        this->vertex_refdt[i]
            = this->GetPoleHeight (i, tau, v2v[i], v2e[i], lh);
      }
}

std::tuple<int, int> TentSlabPitcher::PickNextVertexForPitching (
    const FlatArray<int> &ready_vertices, const FlatArray<double> &ktilde,
...
void TentSlabPitcher::UpdateNeighbours (
    const int vi, const double adv_factor, const Table<int> &v2v,
    const Table<int> &v2e, const FlatArray<double> &tau,
    const BitArray &complete_vertices, Array<double> &ktilde,
    BitArray &vertex_ready, Array<int> &ready_vertices, LocalHeap &lh)
{
  for (int nb : v2v[vi])
    {
...
TentSlabPitcher::InitializeMeshData (LocalHeap &lh,
  ...
  if (calc_local_ct && DIM > 1)
    {
      ...
      this->local_ctau = [this] (const int v, const int el_or_edge) {
        return local_ctau_table[v][el_or_edge];
      };
    }
  else
    {
...
template <int DIM>
double VolumeGradientPitcher<DIM>::GetPoleHeight (const int vi,
                                                  ...
                                                  LocalHeap &lh) const
{
  HeapReset hr (lh);
  constexpr auto el_type = EL_TYPE (DIM);
  // number of vertices of the current element (always the simplex associated
  // to DIM)
...
``        \cdot \nabla \phi_j\right)-\frac{1}{c}^2)
      ...
      const double alpha
          = InnerProduct (gradphi.Row (local_vi), gradphi.Row (local_vi));
      // since alpha>0 we can scale the equation by alpha
      Vec<DIM> tau_i_grad_i = Trans (gradphi) * coeff_vec;
      const double beta
          = 2 * InnerProduct (tau_i_grad_i, gradphi.Row (local_vi)) / alpha;
      const double gamma
          = (InnerProduct (tau_i_grad_i, tau_i_grad_i) - 1.0 / c_max_sq)
            / alpha;
      ...
      const double sol
          = [alpha, beta, gamma, delta, init_pole_height, num_tol] () {
  ...
  // TODO: this checking if a solution was found is quite ugly. how to improve
  // it?
  if (pole_height > 0.75 * init_pole_height)
    return 0.0;
  else
    return pole_height * (1 - num_tol); // just to enforce causality
}
                                                         ...
                                                         const Table<int> &v2e)
{
  ...
  // for a given vertex V in an element E with faces F the constant is
  // calculated as the minimum (over the faces F) ratio between the length of
  // the opposite edge and the biggest edge adjacent to V in F therefore it
  // must be ensured that ctau <=1
  for (auto vi : IntRange (0, n_mesh_vertices))
    {
      if (vi != vmap[vi])
        {
              ...
              if (face_vertices.Pos (vi) == face_vertices.ILLEGAL_POSITION)
                {
                  ...
                  for (auto sl_v : per_verts[vi])
                    {
                      if (face_vertices.Pos (sl_v)
                          == face_vertices.ILLEGAL_POSITION)
                        {
                          found = true;
                        }
                    }
...
template <int DIM>
double EdgeGradientPitcher<DIM>::GetPoleHeight (const int vi,
                                                const FlatArray<double> &tau,
                                                FlatArray<int> nbv,
                                                FlatArray<int> nbe,
                                                LocalHeap &lh) const
  ...
  for (int nb_index : nbv.Range ())
    {
      ...
      const double kt1
          = tau[nb] - tau[vi] + global_ctau * local_ct * length / c_max;
      kt = min (kt, kt1);
    }
  constexpr double num_tol = std::numeric_limits<double>::epsilon ();
...
template <int DIM>
Table<double>
EdgeGradientPitcher<DIM>::CalcLocalCTau (LocalHeap &lh, const Table<int> &v2e)
{
  constexpr auto el_type = EL_TYPE (DIM); // simplex of dimension dim
  constexpr auto n_el_vertices = DIM + 1; // number of vertices of that simplex
  const auto n_mesh_vertices = ma->GetNV ();
          ...
          for (auto iel : edge_els)
            {
                ...
                if (el.Points ().Pos (v) != el.Points ().ILLEGAL_POSITION)
                ...
                else
                  {
                    for (const auto &p_v : per_vs)
                      {
                        if (el.Points ().Pos (p_v)
                            != el.Points ().ILLEGAL_POSITION)
                          {
              ...
              /*the inner products gradphi.edgevec are identically equal to one
                so there is no need to calculate them*/
              const auto v1_local = FindVertexInEl (vmap[v1]);
              const auto v2_local = FindVertexInEl (vmap[v2]);

              /*
                splitting 2d and 3d code. there is no need to generate that
                much code for 2d
              */
              const auto one_over_max_grad = [&] () {
                ...
                if constexpr (DIM == 2)
                  {
                    return 1.0
                           / max (L2Norm (gradphi.Row (v1_local)),
                    ...
                    for (auto face : edge_faces)
                      {
                        ...
                        if (face_local == el.Faces ().ILLEGAL_POSITION)
                          {
                            continue;
                          }
                        ...
                        */
                        auto calc_grad_proj = [&] (const int vertex) {
                          Vec<DIM> max_grad_vec = gradphi.Row (vertex);
                          const double norm_grad
                              = L2Norm (gradphi.Row (vertex));
...
void TentPitchedSlab::DrawPitchedTentsGL (Array<int> &tentdata,
  ...
  for (size_t i = 0; i < ntents; i++)
    {
      ...
      iri[i]
          = new (lh) SIMD_IntegrationRule (fei[i]->ElementType (), 2 * order);
          ...
          if (felpos[i][j] != size_t (-1))
            {
              ...
              if (j == 0)
                {
                  fir[i]
                      = new (lh) SIMD_IntegrationRule (etfacet, 2 * order + 1);
                  fir[i]->SetIRX (nullptr); // quick fix to avoid usage of TP
                                            // elements (slows down)
                }
