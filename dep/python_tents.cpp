...
auto ExportTimeSlab (py::module &m)
{
  ...
  py::class_<TentPitchedSlab, shared_ptr<TentPitchedSlab>> (m, pyname, pydocu)
      .def (
          py::init ([] (shared_ptr<MeshAccess> ma, string method_name,
                        int heapsize) {
            ngstents::PitchingMethod method = [method_name] {
              if (method_name == "edge")
                return ngstents::EEdgeGrad;
              else if (method_name == "vol")
                return ngstents::EVolGrad;
              else // just for static analyzers. the code should not reach this
                   // case
                {
                  cout
                      << "Invalid method! Setting edge algorithm as default..."
                      << endl;
                  return ngstents::EEdgeGrad;
                }
            }();
          ...
          py::arg ("mesh"), py::arg ("method") = "edge",
          py::arg ("heapsize") = 1000000)
      .def_readonly ("mesh", &TentPitchedSlab::ma)
      .def_property_readonly (
          "gradphi",
          [] (shared_ptr<TentPitchedSlab> self) -> shared_ptr<CF> {
            return self->cfgradphi;
          })
      .def ("SetMaxWavespeed",
            [] (shared_ptr<TentPitchedSlab> self, py::object wavespeed) {
              if (auto ws = py::extract<double> (wavespeed); ws.check ())
                self->SetMaxWavespeed (ws ());
              else if (auto ws = py::extract<shared_ptr<CF>> (wavespeed);
                       ws.check ())
                self->SetMaxWavespeed (ws ());
              else
      ...
      .def (
          "PitchTents",
          [] (shared_ptr<TentPitchedSlab> self, const double dt,
            ...
            switch (dim)
              ...
              case 3:
                ...
                break;
              default:
                throw Exception ("TentPitchedSlab not avaiable for dimension "
                                 + ToString (dim));
              }
            return success;
          },
          py::arg ("dt"), py::arg ("local_ct") = false,
          py::arg ("global_ct") = 1.0)
      ...
      .def ("GetTent", &TentPitchedSlab::GetTent,
            pybind11::return_value_policy::reference_internal)
      ////////////////////////////
      // visualization functions
      ////////////////////////////
      .def ("_TentData1D",
            [] (shared_ptr<TentPitchedSlab> self) {
              py::list ret;
      ...
      .def (
          "DrawPitchedTentsVTK",
          [] (shared_ptr<TentPitchedSlab> self, string vtkfilename) {
            if (self->ma->GetDimension () != 2)
              throw Exception (
                  "VTK export is only supported for 2D spatial meshes");
            else
