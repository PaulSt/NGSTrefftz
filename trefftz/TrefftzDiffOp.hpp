...
namespace ngfem
{
  ...
  */
  ...
  public:
    ...
    {
      Cast (fel).CalcShape (
          mip.IP (), mat.Row (0)); // FlatVector<> (fel.GetNDof(), &mat(0,0)));
    }
    ...
    {
      Cast (fel).Evaluate (mir.IR (), x,
                           FlatVector<> (mir.Size (), &y (0, 0)));
    }

    template <class MIR>
    static void
    ApplyIR (const FiniteElement &fel, const MIR &mir, FlatVector<Complex> x,
             FlatMatrix<Complex> y, LocalHeap &lh)
    {
      Cast (fel).Evaluate (
          mir.IR (),
    ...
    {
      Cast (fel).EvaluateTrans (mir.IR (),
                                FlatVector<> (mir.Size (), &x (0, 0)), y);
    }
