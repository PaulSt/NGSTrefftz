...
namespace ngcomp
{
  ...
  public:
    ...
    virtual void
    Apply (const FiniteElement &fel, const SIMD_BaseMappedIntegrationRule &mir,
           BareSliceVector<double> x, BareSliceMatrix<SIMD<double>> flux) const
    {
      if (comp == -1)
        static_cast<const BaseScalarFiniteElement &> (fel).Evaluate (
            mir.IR (), SliceMatrix<double> (fel.GetNDof (), dim, dim, &x (0)),
            flux);
      else
        diffop->Apply (fel, mir, x.Slice (comp, dim),
                       flux.RowSlice (comp, dim));
    }
    virtual void AddTrans (const FiniteElement &fel,
                           const SIMD_BaseMappedIntegrationRule &mir,
                           BareSliceMatrix<SIMD<double>> flux,
                           BareSliceVector<double> x) const
    {
      if (comp == -1)
        static_cast<const BaseScalarFiniteElement &> (fel).AddTrans (
            mir.IR (), flux,
            SliceMatrix<double> (fel.GetNDof (), dim, dim, &x (0)));
      else
      ...
      : FESpace (ama, flags)
  {
    ...
    if (dimension > 1)
      integrator[VOL] = make_shared<BlockBilinearFormIntegrator>
    (integrator[VOL], dimension);
    */
    ...
    if (dimension > 1)
      {
        evaluator[VOL] = make_shared<BlockDifferentialOperatorId> (
            evaluator[VOL], dimension);
        // evaluator[VOL] = make_shared<BlockDifferentialOperator>
        // (evaluator[VOL], dimension);
        flux_evaluator[VOL] = make_shared<BlockDifferentialOperator> (
            flux_evaluator[VOL], dimension);
        // evaluator[BND] = make_shared<BlockDifferentialOperator>
        // (evaluator[BND], dimension);
        /*
        boundary_flux_evaluator =
          make_shared<BlockDifferentialOperator> (boundary_flux_evaluator,
        dimension);
        */
      }

    all_dofs_together = true; // flags.GetDefineFlag ("all_dofs_together");

    // Flags loflags;
    // loflags.SetFlag ("order", 0.0);
    // loflags.SetFlag ("dim", dimension);
    // if (dgjumps){ *testout << "(L2HOFES:)setting loflag dgjumps " << endl;
    // loflags.SetFlag ("dgjumps");} if (iscomplex) loflags.SetFlag
    // ("complex");
    ...
    //     prol = make_shared<ElementProlongation>
    //     (*static_cast<ElementFESpace*> (low_order_space.get()));
    //   }
  }
  ...
  void L2HighOrderFESpace2 ::UpdateCouplingDofArray ()
  {
  ...
  // const FiniteElement & L2HighOrderFESpace2 :: GetFE (int elnr, LocalHeap &
  // lh) const
  // {
  //   try
  //     {
  ...
  //             break; case ET_SEGM:    return * new (lh)
  //             ScalarDummyFE<ET_SEGM> (); break; case ET_TRIG:    return *
  //             new (lh) ScalarDummyFE<ET_TRIG> (); break; case ET_QUAD:
  //             return * new (lh) ScalarDummyFE<ET_QUAD> (); break; case
  //             ET_TET:     return * new (lh) ScalarDummyFE<ET_TET> (); break;
  //             case ET_PYRAMID: return * new (lh) ScalarDummyFE<ET_PYRAMID>
  //             (); break; case ET_PRISM:   return * new (lh)
  //             ScalarDummyFE<ET_PRISM> (); break; case ET_HEX:     return *
  //             new (lh) ScalarDummyFE<ET_HEX> (); break;
  //             }
  //         }
  ...
  //       if (eltype == ET_TET)
  //         return *CreateL2HighOrderFE<ET_TET> (order,
  //         INT<4>(ngel.Vertices()), lh);
  ...
  const FiniteElement &
  L2HighOrderFESpace2 ::GetFacetFE (int fnr, LocalHeap &lh) const
  {
    ...
    switch (vnums.Size ())
      {
      case 1:
        return *new (lh) L2HighOrderFE<ET_POINT> (0);
      case 2:
        return *CreateL2HighOrderFE<ET_SEGM> (order, vnums, lh);
      case 3:
        fe2d = new (lh) L2HighOrderFE<ET_TRIG> ();
        break;
      case 4:
        fe2d = new (lh) L2HighOrderFE<ET_QUAD> ();
        break;
      default:
        {
          stringstream str;
          str << "L2HighOrderFESpace2 " << GetClassName ()
  ...
  // const FiniteElement & L2HighOrderFESpace2 :: GetSFE (int elnr, LocalHeap &
  // lh) const
  // {
  ...
  void L2HighOrderFESpace2 ::GetDofRanges (ElementId ei,
                                           Array<IntRange> &dranges) const
  {
    dranges.SetSize (0);
  ...
  shared_ptr<Table<int>>
  L2HighOrderFESpace2 ::CreateSmoothingBlocks (const Flags &precflags) const
  {
    int i, j, first;
    Array<int> cnt (nel);
    cnt = 0;
  ...
  void L2HighOrderFESpace2 ::GetVertexDofNrs (int vnr, Array<int> &dnums) const
  {
    dnums.SetSize0 ();
  }

  void L2HighOrderFESpace2 ::GetEdgeDofNrs (int ednr, Array<int> &dnums) const
  {
    dnums.SetSize0 ();
  }

  void L2HighOrderFESpace2 ::GetFaceDofNrs (int fanr, Array<int> &dnums) const
  {
    dnums.SetSize0 ();
  }

  void L2HighOrderFESpace2 ::GetInnerDofNrs (int elnr, Array<int> &dnums) const
  {
    GetDofNrs (elnr, dnums);
  }

  void L2HighOrderFESpace2 ::SolveM (CoefficientFunction &rho, BaseVector &vec,
                                     LocalHeap &lh) const
  {
    ...
    //                      {
    //                      case 1:
    //                        static_cast<const DGFiniteElement<1>&>
    //                        (fel).GetDiagMassMatrix (diag_mass);
    //                      case 2:
    //                        static_cast<const DGFiniteElement<2>&>
    //                        (fel).GetDiagMassMatrix (diag_mass);
    //                      case 3:
    //                        static_cast<const DGFiniteElement<3>&>
    //                        (fel).GetDiagMassMatrix (diag_mass);
    //                      }
    //
    ...
    //                        2*fel.Order()); BaseMappedIntegrationRule & mir =
    //                        trafo(ir, lh); FlatVector<> pntvals(ir.GetNIP(),
    //                        lh);
    //
    //                        for (int i = 0; i < melx.Height(); i++)
    //                          melx.Row(i) /= diag_mass(i);
    ...
    //                          {
    //                            static_cast<const BaseScalarFiniteElement&>
    //                            (fel).Evaluate (ir, melx.Col(comp), pntvals);
    //                            for (int i = 0; i < pntvals.Size(); i++)
    //                              pntvals(i) *= ir[i].Weight() /
    //                              mir[i].GetMeasure();
    //                            static_cast<const BaseScalarFiniteElement&>
    //                            (fel).EvaluateTrans (ir, pntvals,
    //                            melx.Col(comp));
    //                          }
    //                        for (int i = 0; i < melx.Height(); i++)
    //                          melx.Row(i) /= diag_mass(i);
    //                        */
    //                        SIMD_IntegrationRule ir(fel.ElementType(),
    //                        2*fel.Order()); auto & mir = trafo(ir, lh);
  ...
  L2SurfaceHighOrderFESpace ::L2SurfaceHighOrderFESpace (
      shared_ptr<MeshAccess> ama, const Flags &flags, bool parseflags)
      : FESpace (ama, flags)
  {
    ...
    if (flags.NumFlagDefined ("relorder"))
      throw Exception (
          "Variable order not implemented for L2SurfaceHighOrderFESpace");
    ...
    if (ma->GetDimension () == 2)
      {
        integrator[BND] = make_shared<RobinIntegrator<2>> (
            make_shared<ConstantCoefficientFunction> (1));
        evaluator[BND]
            = make_shared<T_DifferentialOperator<DiffOpIdBoundary<2>>> ();
        evaluator[VOL]
            = make_shared<T_DifferentialOperator<DiffOpId<2>>> (); // for
                                                                   // dimension
      }
    else
      {
        integrator[BND] = make_shared<RobinIntegrator<3>> (
            make_shared<ConstantCoefficientFunction> (1));
        evaluator[BND]
            = make_shared<T_DifferentialOperator<DiffOpIdBoundary<3>>> ();
        evaluator[VOL]
            = make_shared<T_DifferentialOperator<DiffOpId<3>>> (); // for
                                                                   // dimension
      }

    if (dimension > 1)
      integrator[BND] = make_shared<BlockBilinearFormIntegrator> (
          integrator[BND], dimension);
  }

  L2SurfaceHighOrderFESpace ::~L2SurfaceHighOrderFESpace () { ; }

  shared_ptr<FESpace>
  L2SurfaceHighOrderFESpace ::Create (shared_ptr<MeshAccess> ma,
                                      const Flags &flags)
  {
  ...
  // const FiniteElement & L2SurfaceHighOrderFESpace :: GetSFE (int elnr,
  // LocalHeap & lh) const
  // {
  //   if (ma->GetDimension() == 2)
  //     {
  ...
  //   dynamic_cast<L2HighOrderFiniteElement<2>*> (fe) -> SetVertexNumbers
  //   (vnums);
  ...
  FiniteElement &
  L2SurfaceHighOrderFESpace ::GetFE (ElementId ei, Allocator &lh) const
  {
    if (ei.VB () == BND)
      {
        if (ma->GetDimension () == 2)
          {
            ...
            switch (ngel.GetType ())
              {
              case ET_SEGM:
                fe1d = new (lh) L2HighOrderFE<ET_SEGM> ();
                break;
              default:;
              }
  ...
  // const FiniteElement & L2SurfaceHighOrderFESpace :: GetFE (int elnr,
  // LocalHeap & lh) const
  // {
  //   throw Exception ("Volume elements not available for
  //   L2SurfaceHighOrderFESpace");
  // }

  size_t L2SurfaceHighOrderFESpace ::GetNDof () const throw () { return ndof; }

  void
  L2SurfaceHighOrderFESpace ::GetDofNrs (ElementId ei, Array<int> &dnums) const
  {
    dnums.SetSize (0);
    if (ei.VB () != BND)
      return;
  ...
  void L2SurfaceHighOrderFESpace ::GetVertexDofNrs (int vnr,
  ...
  void
  L2SurfaceHighOrderFESpace ::GetEdgeDofNrs (int ednr, Array<int> &dnums) const
  {
    dnums.SetSize (0);
    return;
  }
  ...
  void
  L2SurfaceHighOrderFESpace ::GetFaceDofNrs (int fanr, Array<int> &dnums) const
  {
    GetDofNrs (fanr, dnums);
    return;
  }

  void L2SurfaceHighOrderFESpace ::GetInnerDofNrs (int elnr,
                                                   Array<int> &dnums) const
  {
    GetDofNrs (elnr, dnums);
    return;
  ...
  namespace l2hofespace_cpp
  {
    ...
    Init::Init ()
    {
      ...
      GetFESpaceClasses ().AddFESpace ("l22ho", L2HighOrderFESpace2::CreateHO);
      GetFESpaceClasses ().AddFESpace ("l22surf",
                                       L2SurfaceHighOrderFESpace::Create);
    }
