...
namespace ngfem
{
  ...
  /*
    ...
    void MappedElement ::
    Evaluate (const IntegrationRule & ir, BareSliceVector<double> coefs,
    ...
    void MappedElement ::
    Evaluate (const SIMD_IntegrationRule & ir, BareSliceVector<> coefs,
    BareVector<SIMD<double>> values) const
    {
      throw ExceptionNOSIMD (string("Evaluate (simd) not implemented for class
    ")+typeid(*this).name());
    }

    void MappedElement ::
    Evaluate (const SIMD_IntegrationRule & ir, SliceMatrix<> coefs,
    ...
    void MappedElement ::
    Evaluate (const IntegrationRule & ir, SliceMatrix<> coefs, SliceMatrix<>
    values) const
    {
    ...
    void MappedElement ::
    AddTrans (const SIMD_IntegrationRule & ir, BareSliceMatrix<SIMD<double>>
    values, SliceMatrix<> coefs) const
    {
