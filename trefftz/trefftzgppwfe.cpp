...
namespace ngfem
{
    ...
    template<int D>
    void TrefftzGppwFE<D> :: CalcMappedDDShape (const BaseMappedIntegrationPoint & bmip,
    ...
    {
        {
            ...
            if ( gtbstore[encode][0].Size() == 0)
            {
                ...
                const int nbasis2 = (BinCoeff(D + ord, ord) + BinCoeff(D + ord-1, ord-1));
                ...
                {
                    int tracker = 0;
                    TB_inner(ord, trefftzbasis2, coeff, b, D+1, tracker, basistype);
                }
                        ...
                        for(int ell=j-1;ell<=gppword-2;ell++)
                        {
                            ...
                            for(int t=0;t<=ell;t++)
                            {
                                ...
                                trefftzbasis( basisn, IndexMap2(get_coeff, gppword)) =
                                    ...
                                    * trefftzbasis( basisn, IndexMap2(get_coeff2, gppword))
                                    //- (t<=j-2)*BinCoeff(j,t+2) * gamma[x+t+2-j]*pow(gamma[0],0.5*(j-t-4))
                                    //* pow(TrefftzGppwFE<D>::GetDirection(j,dir),x)
                                    //+ (x+t+2<=ord)*trefftzbasis2( basisn, IndexMap2(get_coeff2, ord))
                                    ;
                                ...
                                for(int betax=max(0,j-t-1);betax<x;betax++)
                                {
                                    ...
                                    get_coeff2[0] = betax;
                                    trefftzbasis( basisn, IndexMap2(get_coeff, gppword))
                                        -= gamma[x-betax]*trefftzbasis( basisn, IndexMap2(get_coeff2, gppword)) / gamma[0];
                                }
                                for(int betax=0;betax<x;betax++)
                                {
                                    ...
                                    get_coeff2[0] = betax;
                                    trefftzbasis( basisn, IndexMap2(get_coeff, gppword))
                                        -= (betax+t+2<=ord)*gamma[x-betax]*trefftzbasis2( basisn, IndexMap2(get_coeff2, ord));
                                }
                            }
                        }
                        basisn++;
    ...
    template<int D>
    void TrefftzGppwBasis<D> :: TB_inner(int ord, Matrix<> &trefftzbasis, Vec<D+1, int> coeffnum, int basis, int dim, int &tracker, int basistype)
    {
        if (dim>0)
        {
