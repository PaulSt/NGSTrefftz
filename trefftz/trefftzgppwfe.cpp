...
namespace ngfem
{
    ...
    template<int D>
    void TrefftzGppwBasis<D> :: CreateTB(int ord, int gppword, Vector<> gamma, int basistype)
    {
                ...
                for(int ell=(basisn++)-1;ell<gppword;ell++)
                {
                    ...
                    for(int t=0;t<ell;t++) 
                    {
                        ...
                        trefftzbasis( basisn, IndexMap2(get_coeff, gppword-1)) =
                            (x+2)*(x+1)/(t+2)*(t+2) * trefftzbasis( basisn, IndexMap2(get_coeff2, gppword-1))
    ...
    template<int D>
    void TrefftzGppwBasis<D> :: TB_inner(Vector<> gamma, int ord, Matrix<> &trefftzbasis, Vec<D+1, int> coeffnum, int basis, int dim, int &tracker, int basistype)
    {
        //if (dim>0)
        //{
