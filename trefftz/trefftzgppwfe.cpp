...
namespace ngfem
{
                                        ...
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        for (int imip = 0; imip < smir.Size(); imip++)
        {
            ...
            const CSR* localmat = TrefftzGppwBasis<2>::getInstance().TB(ord);
                                        ...
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        for (int imip = 0; imip < smir.Size(); imip++)
        {
            ...
            // TB*monomials for trefftz shape fcts
            const CSR* localmat = TrefftzGppwBasis<3>::getInstance().TB(ord);
            for (int i=0; i<nbasis; ++i)
            {
                shape(i,imip) = 0.0;
                for (int j=(*localmat)[0][i]; j<(*localmat)[0][i+1]; ++j)
    ...
    template<>
    void TrefftzGppwFE<4> :: CalcShape (const SIMD_BaseMappedIntegrationRule & smir,
                                        BareSliceMatrix<SIMD<double>> shape) const
    {
        ...
        for (int imip = 0; imip < smir.Size(); imip++)
        {
            ...
            // TB*monomials for trefftz shape fcts
            const CSR* localmat = TrefftzGppwBasis<4>::getInstance().TB(ord);
            for (int i=0; i<nbasis; ++i)
            {
                shape(i,imip) = 0.0;
                for (int j=(*localmat)[0][i]; j<(*localmat)[0][i+1]; ++j)
    ...
    template<>
    void TrefftzGppwFE<4> :: CalcDShape (const SIMD_BaseMappedIntegrationRule & smir,
                                         BareSliceMatrix<SIMD<double>> dshape) const
    {
        for (int imip = 0; imip < smir.Size(); imip++)
        {
    ...
    template<>
    void TrefftzGppwFE<4> :: CalcShape (const BaseMappedIntegrationPoint & mip,
                                        BareSliceVector<> shape) const
    {
    ...
    template<>
    void TrefftzGppwFE<4> :: CalcDShape (const BaseMappedIntegrationPoint & mip,
                                         SliceMatrix<> dshape) const
    {
        Vec<4> cpoint = mip.GetPoint();
        cpoint -= elcenter; cpoint *= (2.0/elsize); cpoint[3] *= c;
    ...
    template class TrefftzGppwFE<4>;
    ...
    template class TrefftzGppwBasis<4>;
