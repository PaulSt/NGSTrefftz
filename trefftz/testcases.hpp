...
namespace ngcomp
{
  ...
  template <int D> Vec<D + 2> simpleexp (Vec<D + 1> p, double wavespeed)
  {
    double x = p[0];
    double t = p[D];
    Vec<D + 2> sol;
  ...
  template <int D> Vec<D + 2> standingwave (Vec<D + 1> p, double wavespeed)
  {
    ...
    if (D == 1)
      {
        sol[0]
            = cos (M_PI * x) * sin (M_PI * t * sqrt (2)) / (sqrt (2) * M_PI);
      ...
      {
        double y = p[1];
        sol[0] = cos (M_PI * x) * cos (M_PI * y) * sin (M_PI * t * sqrt (2))
                 / (sqrt (2) * M_PI);
        sol[1] = -sin (M_PI * x) * cos (M_PI * y) * sin (M_PI * t * sqrt (2))
                 / sqrt (2);
        sol[2] = -cos (M_PI * x) * sin (M_PI * y) * sin (M_PI * t * sqrt (2))
                 / sqrt (2);
    ...
    else if (D == 3)
      {
        ...
        sol[0] = cos (M_PI * x) * cos (M_PI * y) * cos (M_PI * z)
                 * sin (M_PI * t * sqrt (2)) / (sqrt (2) * M_PI);
        sol[1] = -sin (M_PI * x) * cos (M_PI * y) * cos (M_PI * z)
                 * sin (M_PI * t * sqrt (2)) / sqrt (2);
