...
namespace ngcomp
{
    ...
    template<int D>
    Vec<D+2> simplesin(Vec<D+1> p, double wavespeed)
    {
        ...
        if(D==1){
            ...
            sol[2] = sin(M_PI*x)*cos(M_PI*t*wavespeed)*wavespeed*M_PI;
    ...
    Vec<D+2> gausspw(Vec<D+1> p, double wavespeed)
    {
        ...
        if(D==1){
            ...
            double y = p[1];
            sol[0] = exp(-delta*((x-0.5)*(x-0.5)+(y-0.5)*(y-0.5)) );
            sol[1] = -2*delta * (x-0.5) * sol[0];
            ...
            double y = p[1];
            double z = p[2];
            sol[0] = exp(-delta*((x-0.5)*(x-0.5)+(y-0.5)*(y-0.5)+(z-0.5)*(z-0.5)) );
            sol[1] = -2*delta * (x-0.5) * sol[0];
            sol[2] = -2*delta * (y-0.5) * sol[0];
    ...
    Vec<D+2> vertgausspw(Vec<D+1> p, double wavespeed)
    {
        ...
        if(D==1){
            ...
            double y = p[1];
            sol[0] = exp(-delta*((x-0.25)*(x-0.25)) );
            sol[1] = -2*delta * (x-0.25) * sol[0];
            sol[2] = 0;
            sol[3] = 0;
        } else if(D==3) {
            double y = p[1];
    ...
    template<int D>
    Vec<D+2> standingwave(Vec<D+1> p, double wavespeed)
    {
        double x = p[0]; double t = p[D];
        Vec<D+2> sol;
        int k = 1;
