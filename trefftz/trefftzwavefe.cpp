...
namespace ngfem
{
    ...
    for (int imip = 0; imip < smir.Size (); imip++)
      {
        ...
        for (int i = 0; i < (*localmat)[0].Size (); i++)
          tempshape[(*localmat)[0][i]]
              += (*localmat)[2][i] * pol[(*localmat)[1][i]];
        for (int b = 0; b < nbasis; b++)
          shape.Col (imip) (b) = tempshape (b);
      }
  }
    ...
    for (int imip = 0; imip < smir.Size (); imip++)
      {
        ...
        // SIMD<double> pol = polxt[0][i-(d==0)] * polxt[1][j-(d==1)] *
        // (d==0?i:j); tempdshape += pol * localmat.Col(ii-1);
        // }
                                     ...
                                     SliceMatrix<> dshape) const
  {
    ...
    // polxt[2][k-(d==2)] * (d==0?i:(d==1?j:k)); tempdshape += pol *
    // localmat.Col(ii-1);
    // }
    // dshape.Col(d) = tempdshape;
    // }
  ...
  template <int D> const coo *TrefftzWaveBasis<D>::TB (int ord)
  {
    {
