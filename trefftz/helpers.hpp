      ...
      for (int j = ord; j > 0; j--)
        for (int i = 1; i <= D; i++)
          for (int k = 1; k <= pascal (j - 1, i - 1); k++)
            {
              if (coeff.Size ()
                  < pascal (j - 1, D) + pascal (j, i - 2) + k - 1)
                {
                  // cout << pascal << endl;
                  // cout << coeff.Size() << endl;

                  // cout << "D: " << D << " ord: " << ord <<" j: " << j << "
                  // i: " << i << " k: " << k<< " index1: " <<pascal(j-2,D)+k
                  // << " index2: " <<  pascal(j-1,D)+pascal(j,i-2)+k << endl<<
                  // endl;
                } // coeff( pascal(j-2,D)+k ) += point( D-i+1 ) * coeff(
                  // pascal(j-1,D)+pascal(j,i-2)+k ); coeff(
                  // pascal(j-1,D)+pascal(j,i-2)+k ) = 0; cout << coeff <<
                  // endl;
            }
    }
