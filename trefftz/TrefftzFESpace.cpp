...
namespace ngcomp
{
          ...
          case 1:
            {
              ...
              // evaluator[BND] =
              // make_shared<T_DifferentialOperator<DiffOpMappedBoundary<2,TrefftzElement<1,3>>>>();
              break;
            }
          case 2:
            {
              ...
              // evaluator[BND] =
              // make_shared<T_DifferentialOperator<DiffOpMappedBoundary<2,TrefftzElement<2,3>>>>();
              break;
            }
          case 3:
            {
              ...
              // evaluator[BND] =
              // make_shared<T_DifferentialOperator<DiffOpMappedBoundary<3,
              // TrefftzElement<2,3>>>>();
              break;
            }
          }
  ...
  FiniteElement &TrefftzFESpace ::GetFE (ElementId ei, Allocator &alloc) const
  {
    ...
    if (order == 3)
          ...
          case ET_TET:
            {
              ...
              return *(new (alloc) TrefftzElement<3, 3> (ET_TET))
                          ->SetWavespeed (c); // ->SetCenter(center)
    ...
    else
      {
        switch (ma->GetElType (ei))
          ...
          case ET_QUAD:
            {
              return *(new (alloc) T_TrefftzElement<2> (order, ET_QUAD))
                          ->SetWavespeed (c);
              break;
            }
          case ET_TRIG:
          ...
          case ET_PYRAMID:
            {
              return *(new (alloc)
                           T_TrefftzElement<3> (order, ma->GetElType (ei)))
                          ->SetWavespeed (c);
              break;
            }
          case ET_TET:
            {
              ...
              for (auto vertex : vertices_index)
                center += ma->GetPoint<3> (vertex);
              center *= 0.25;
              return *(new (alloc) T_TrefftzElement<3> (order, ET_TET))
                          ->SetWavespeed (c); // ->SetCenter(center)
                                              // ->SetElSize( Adiam<3>(ei) );
              break;
            }
    ...
    // if (ei.VB()==BBND) throw Exception ("BBND not available in
    // L2HighOrderFESpace2"); if (ei.IsVolume())
    // {
    // 	int elnr = ei.Nr();
    ...
    // 		switch (eltype)
    // 		{
    // 			case ET_POINT:   return * new (alloc) ScalarDummyFE<ET_POINT> ();
    // break; 			case ET_SEGM:    return * new (alloc) ScalarDummyFE<ET_SEGM> ();
    // break; 			case ET_TRIG:    return * new (alloc) ScalarDummyFE<ET_TRIG> ();
    // break; 			case ET_QUAD:    return * new (alloc) ScalarDummyFE<ET_QUAD> ();
    // break; 			case ET_TET:     return * new (alloc) ScalarDummyFE<ET_TET> ();
    // break; 			case ET_PYRAMID: return * new (alloc) ScalarDummyFE<ET_PYRAMID>
    // (); break; 			case ET_PRISM:   return * new (alloc) ScalarDummyFE<ET_PRISM>
    // (); break; 			case ET_HEX:     return * new (alloc) ScalarDummyFE<ET_HEX>
    // (); break;
    // 		}
    // 	}
    //
    ...
    //
    // 		default:
    // 			throw Exception ("illegal element in
    // L2HoFeSpace::GetFE");
    // 	}
    // }
    // else
