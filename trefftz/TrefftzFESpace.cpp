...
namespace ngcomp
{
  ...
  FiniteElement &TrefftzFESpace ::GetFE (ElementId ei, Allocator &alloc) const
  {
    ...
    // 	{
    // 		Vec<2> center = 0;
    // 		for(auto vertex : vertices_index) center +=
    // ma->GetPoint<2>(vertex); 		center *= (1.0/3.0); 		return *(new (alloc)
    // TrefftzElement<2,3>(ET_TRIG))
    // ->SetWavespeed(c)->SetCenter(center)->SetElSize( Adiam<2>(ei) ); 		break;
    // 	}
    // 	case ET_HEX:
    // 	case ET_PRISM:
    // 	case ET_PYRAMID:
    ...
    // 		Vec<3> center = 0;
    // 		for(auto vertex : vertices_index) center +=
    // ma->GetPoint<3>(vertex); 		center *= 0.25; 		return *(new (alloc)
    // TrefftzElement<3,3>(ET_TET)) ->SetWavespeed(c);// ->SetCenter(center)
    // ->SetElSize( Adiam<3>(ei) ); 		break;
    // 	}
    // }
