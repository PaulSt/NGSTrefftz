...
namespace ngcomp
{
    ...
    : FESpace (ama, flags)
  {
    ...
    order = int(flags.GetNumFlag ("order", 2));//flags.GetDefineFlag ("order");
		...
		switch (D) {
			case 2:
			{
		    evaluator[VOL] = make_shared<T_DifferentialOperator<DiffOpMapped<2,3>>>();
				evaluator[BND] = make_shared<T_DifferentialOperator<DiffOpMappedBoundary<2,3>>>();
				break;
			}
			case 3:
			{
  ...
  void TrefftzFESpace :: Update(LocalHeap & lh)
  {
		...
		cout << "update: order = " << order<< " ndof = " <<  ndof << " local_ndof:" << local_ndof << endl;
	}
  ...
  FiniteElement & TrefftzFESpace :: GetFE (ElementId ei, Allocator & alloc) const
  {
		...
		//cout << "element vectice coord: \n"  << ma->GetPoint<3>(bla[0]) << endl<< ma->GetPoint<3>(bla[1]) <<endl<<ma->GetPoint<3>(bla[2])<<endl<<ma->GetPoint<3>(bla[3])<<endl;

		switch (D) {
			case 2:
			{
				Vec<2> center = 0;
				for(int d=0;d<2;d++) center += ma->GetPoint<2>(vertices_index[d]);
				center *= 0.25;
			...
			{
				Vec<3> center = 0;
				for(int d=0;d<D;d++) center += ma->GetPoint<3>(vertices_index[d]);
				center *= 0.25;
