...
namespace ngcomp
{
    ...
    if (ei.IsVolume ())
      {
        switch (ma->GetElType (ei))
          ...
          case ET_SEGM:
            {
              throw Exception (
                  "illegal element type in Trefftz::GetSurfaceFE");
              break;
            }
          case ET_QUAD:
          case ET_TRIG:
  ...
  template <int D>
  double MonomialFESpace ::Adiam (ElementId ei,
  ...
  template <int D> Vec<D + 1> MonomialFESpace ::ElCenter (ElementId ei) const
  {
    Vec<D + 1> center = 0;
    auto vertices_index = ma->GetElVertices (ei);
