...
namespace ngfem
{
		...
		for(int l=0;l<nbasis;l++) //loop over basis functions
		{
			for(int d=0;d<D;d++)  //loop over derivatives/dimensions
			{
				for(int i=0;i<BinCoeff(D+1 + order, order);i++)//loop over indices
				{
					...
					{
						tempexp = indices[i];
						dshape(l,d) += tempexp[d+1]-- * basisFunctions[l].get(indices[i]) * ipow_ar(ip,tempexp);
					}
				}
			}
		}
                                  ...
                                  BareSliceVector<> shape) const
  {
		for(int l=0;l<nbasis;l++) //loop over basis functions
		{
			for(int i=0;i<BinCoeff(D+1 + order, order);i++)//loop over indices
			{
				shape(l) += basisFunctions[l].get(indices[i]) * ipow_ar(mip.GetPoint(),indices[i]);
			}
		}
	}
		...
		for(int l=0;l<nbasis;l++) //loop over basis functions
		{
			for(int d=0;d<D;d++)  //loop over derivatives/dimensions
			{
				for(int i=0;i<BinCoeff(D+1 + order, order);i++)//loop over indices
				{
					...
					{
						tempexp = indices[i];
						dshape(l,d) += tempexp[d+1]-- * basisFunctions[l].get(indices[i]) * ipow_ar(mip.GetPoint(),tempexp);
					}
				}
			}
		}
	...
	template <int D, int order>
	float TrefftzElement<D,order> :: ipow_ar(IntegrationPoint base, array<int, D+1> exp) const {
		int result = 1;
		for(int i=0;i<D+1;i++){
	...
	template <int D, int order>
	float TrefftzElement<D,order> :: ipow_ar(FlatVector<double> base, array<int, D+1> exp) const {
		int result = 1;
		for(int i=0;i<D+1;i++){
			result *= pow(base(i),exp[i]);
		}
	...
	template <int D, int order>
	int TrefftzElement<D,order> :: GetNBasis()
	{
		return nbasis;
	}
