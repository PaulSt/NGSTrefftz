...
namespace ngfem
{
    ...
    template<int D>
    TrefftzHeatFE<D> :: TrefftzHeatFE(int aord, float ac, Vec<(D+1)> aelcenter, double aelsize, ELEMENT_TYPE aeltype, int abasistype)
    ...
    template<int D>
    const CSR TrefftzHeatBasis<D> :: TB(int ord)
    {
        return tbstore[ord];
    }

    template<int D>
    void TrefftzHeatBasis<D> :: CreateTB(int ord, int basistype)
    {
    ...
    template<int D>
    void TrefftzHeatBasis<D> :: TB_inner(int ord, Matrix<> &trefftzbasis, Vec<D+1, int> coeffnum, int basis, int dim, int &tracker, int basistype, double wavespeed)
    {
        if (dim>0)
        {
            ...
            if(sum<=ord)
            {
                ...
                else if(coeffnum(D)>0 && coeffnum(D)<basis)
                {
                    for(int m=0;m<D;m++) //rekursive sum
                    {
                        ...
                        if(get_coeff[D] + get_coeff[m] > ord) continue;
                        trefftzbasis( basis, indexmap) += (coeffnum(m)+1.0) * (coeffnum(m)+2.0) * trefftzbasis(basis, IndexMap2(get_coeff, ord));
                //cout << ord << " sim " << sum << " coeff " << coeffnum << " mapped to " << IndexMap2(get_coeff, ord) << " trying to get " << get_coeff << endl <<
                     //" full row " << endl << trefftzbasis.Row(basis) << endl;
                    }
    ...
    template<int D>
    TrefftzHeatTestFE<D> :: TrefftzHeatTestFE(int aord, float ac, Vec<(D+1)> aelcenter, double aelsize, ELEMENT_TYPE aeltype, int abasistype)
    : ScalarMappedElement<D+1>(BinCoeff(D + aord, aord), aord),
    eltype(aeltype),
    basistype(abasistype)
    {
    ...
    template<int D>
    const CSR TrefftzHeatTestBasis<D> :: TB(int ord)
    {
        return tbstore[ord];
    }
    ...
    template<int D>
    void TrefftzHeatTestBasis<D> :: TB_inner(int ord, Matrix<> &trefftzbasis, Vec<D+1, int> coeffnum, int basis, int dim, int &tracker, int basistype, double wavespeed)
    {
        if (dim>0)
        {
            while(coeffnum(dim-1)<=ord)
