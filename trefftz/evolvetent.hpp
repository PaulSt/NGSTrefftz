...
namespace ngcomp
{
  ...
  public:
    ...
    // this->GG.Append(Matrix<>(this->order,this->order));
    ////this->BB.SetSize(0);
    // for(int i=0;i<ama->GetNV();i++)
    // this->BB.Append(Matrix<>(this->order-1)); MappedIntegrationPoint<D,D>
    // mip(ir[0], ama->GetTrafo (ElementId(0), lh)); for(int
    // nx,count=0;nx<this->order;nx++)
    //{
    // for(int ny=0;ny<=(this->order-1)*(D==2);ny++)
    //{
