...
namespace ngcomp
{
  int EvolveTents(shared_ptr<MeshAccess> ma)
  {
    ...
    { 
      ...
      auto pindbot = mesh -> AddPoint(netgen::Point3d(point(0),tent->tbot,0));
      auto pindtop = mesh -> AddPoint(netgen::Point3d(point(0),tent->ttop,0));
      for (int k = 0; k < tent->nbv.Size(); k++)
      {
        point = ma->GetPoint<1>(tent->nbv[k]);
        nbvn[k] = mesh -> AddPoint(netgen::Point3d(point(0),tent->nbtime[k],0));
        cout << k << endl;
      }
      cout << pindbot << endl;
      // if(tent->nbv.Size() == 1)
      ...
      // else if(tent->nbv.Size() == 2)
        // mesh -> AddSurfaceElement(netgen::Element2d(pindbot,pindtop,nbvn[0],nbvn[1]));
      // cout << *tent << endl;
    }
...
{
	m.def("EvolveTents", [](shared_ptr<MeshAccess> ma) -> int 
					{
