...
namespace ngcomp
{
                        ...
                        double dt, Vector<> wavefront)
  {
    ...
    RunParallelDependency (tps.tent_dependency, [&] (int i) {
      ...
      bool bdtent = false;
      for (ElementIterator elit = bd_points.begin (); elit != bd_points.end ();
           ++elit)
        {
          for (auto v : (*elit).Vertices ())
            {
              if (v == tent->vertex && D == 1)
              ...
              else if (v == tent->vertex && D == 2)
                {
                  cout << (*elit).Nr () << endl;
                  Mat<D + 1, D + 1> verts;
                  verts.Col (0).Range (0, D)
                      = ma->GetPoint<D> ((*elit).Vertices ()[0]);
                  verts.Col (1).Range (0, D)
                      = ma->GetPoint<D> ((*elit).Vertices ()[1]);
