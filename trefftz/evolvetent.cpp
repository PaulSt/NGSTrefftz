...
namespace ngcomp
{
                    ...
                    Vector<double> wavefront)
  {
    ...
    RunParallelDependency (tps.tent_dependency, [&] (int i) {
      ...
      cout << "vertex: " << tent.vertex << ", tbot = " << tent.tbot
           << ", ttop = " << tent.ttop << endl;
      ...
      for (auto el : tent.els)
        {
          ...
          cout << "ns: " << bs << endl;
          // cout << "mir0: " << mir[0] << " val: " << faceint.Evaluate( ir[0],
          // bs) << endl; cout << "mir1: " << mir[1] << " val: " <<
          // faceint.Evaluate( ir[1], bs) << endl; cout << "mir1: " <<
          // mir[ir.Size()-1]<<mir[ir.Size()-1].GetWeight() << endl;
          ...
          cout << "edge verts: " << ma->GetPoint<1> (verts[0])
               << ma->GetPoint<1> (verts[1]) << endl;
        }
    });
  ...
  //cout << "v1: " << v1 << " v1 part: " << v1(1,D-1) << "norm " << L2Norm(v1)
  << endl ; anisotropicdiam = max( anisotropicdiam, sqrt( L2Norm2(v1(0,D-2) -
  v2(0,D-2)) + pow(c*(v1(D-1)-v2(D-1)),2) ) );
  }
  }
