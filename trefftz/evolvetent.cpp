          ...
          for (int imip = 0; imip < mir.Size (); imip++)
            {
                  ...
                  for (int j = 0; j < nbasis; j++)
                    {
                      elmatrix (i, j)
                          += (dshape (i, D) * dshape (j, D) * n (D))
                             ...
                             * ir[imip].Weight ();
                      elmatrix (i, j) += (Dot<D> (dshape.Row (i).Range (0, D),
                                                  dshape.Row (j).Range (0, D))
                                          * n (D))
                                         * A * ir[imip].Weight ();
                      elmatrix (i, j)
                          += (dshape (i, D)
                              * Dot<D> (dshape.Row (j).Range (0, D),
                                        n.Range (0, D)))
                             * A * ir[imip].Weight ();
                      elmatrix (i, j)
                          += (dshape (j, D)
                              * Dot<D> (dshape.Row (i).Range (0, D),
                                        n.Range (0, D)))
                             * A * ir[imip].Weight ();
                    }
                }
              ...
              for (int j = 0; j < nbasis; j++)
                {
                  // elvector(j) += (1/(wavespeed*wavespeed))* (
                  // wavefront(ir_order * ma->GetNEdges() * (D+2)) )
                  // *A*ir[imip].Weight();
                }
            }
          cout << elmatrix << endl;
