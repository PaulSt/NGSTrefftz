                  ...
                  for (int j = 0; j < nbasis; j++)
                    {
                      ...
                      elmat (j, i) += (dshape (i, D) * dshape (j, D) * n (D))
                                      * (1 / (wavespeed * wavespeed)) * A
                                      * ir[imip].Weight ();
                      elmat (j, i) += (InnerProduct (sig, tau) * n (D)) * A
                                      * ir[imip].Weight ();
                      elmat (j, i) += (dshape (i, D)
                                       * InnerProduct (tau, n.Range (0, D)))
                                      * A * ir[imip].Weight ();
                      elmat (j, i) += (dshape (j, D)
              ...
              for (int j = 0; j < nbasis; j++)
                {
                  ...
                  elvec (j)
                      -= (wavefront (offset + D + 1) * dshape (j, D) * n (D))
                         * (1 / (wavespeed * wavespeed)) * A
                         * ir[imip].Weight ();
                  elvec (j) -= (InnerProduct (wavefront.Range (offset + 1,
                                                               offset + D + 1),
                                              tau)
                                * n (D))
                               * A * ir[imip].Weight ();
                  elvec (j) -= (wavefront (offset + D + 1)
                                * InnerProduct (tau, n.Range (0, D)))
                               * A * ir[imip].Weight ();
                  elvec (j)
                      -= (dshape (j, D)
                  ...
                  for (int i = 0; i < nbasis; i++)
                    {
                      elmat (j, i)
                          += (shape (i) * shape (j)) * A * ir[imip].Weight ();
                    }
                }
            }
          ...
          for (int imip = 0; imip < ir.Size (); imip++)
            {
              ...
              for (int i = 0; i < nbasis; i++)
                for (int j = 0; j < nbasis; j++)
                  {
                    Vec<D> sig = -dshape.Row (i).Range (0, D);
                    elmat (j, i) += InnerProduct (sig, n.Range (0, D))
