...
namespace ngcomp
{
                    ...
                    Vector<double> wavefront)
  {
    ...
    RunParallelDependency (tps.tent_dependency, [&] (int i) {
      ...
      cout << "vertex: " << tent.vertex
           << " at: " << ma->GetPoint<D> (tent.vertex)
           << ", tbot = " << tent.tbot << ", ttop = " << tent.ttop << endl;
      cout << "neighbour vertices: " << endl;
      for (int k = 0; k < tent.nbv.Size (); k++)
        cout << k << ": " << tent.nbv[k]
             << " at: " << ma->GetPoint<D> (tent.nbv[k])
             << " t: " << tent.nbtime[k] << endl;
          ...
          for (int imip = 0; imip < mir.Size (); imip++)
            {
              ...
              MappedIntegrationPoint<D + 1, D + 1> tmip (
                  ip, FE_ElementTransformation<D + 1, D + 1> ());
